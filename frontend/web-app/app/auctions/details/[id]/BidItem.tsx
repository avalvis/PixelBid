...
export default function BidItem({ bid }: Props) {
    ...
    `}>
            <div className='flex flex-col'>
                <span className='font-bold text-lg'>Bidder: {bid.bidder}</span>
                <span className='text-gray-700 text-sm'>
                    Placed on: {format(new Date(bid.bidTime), 'dd MMM yyyy h:mm a')}
                </span>
            </div>
            <div className='flex flex-col text-right'>
                <div className='text-xl font-semibold'>${numberWithCommas(bid.amount)}</div>
                <div className='flex flex-row items-center'>
                    <span className='font-mono'>{getBidInfo().text}</span>
                </div>
            </div>
        </div>
    )
